...
TEST(String, ReplaceString) {
  ...
  ASSERT_EQ(" ab c  d   efghjijookkjkk//gj\\*&^xyz  end  ", ReplaceFirst(str,
            "kk", "oo"));
  ASSERT_EQ(" ab c  d   efghjijkkkkjkk//gj\\*&^xyz  end  ", ReplaceFirst(str, "",
            "xxxx"));
  ASSERT_EQ(" ab c  d   efghjijoooojoo//gj\\*&^xyz  end  ", ReplaceAll(str, "kk",
            "oo"));
  ASSERT_EQ(" ab c  d   efXhjijkkkkjkk//Xj\\X&^Xyz  end  ", StripString(str,
            "gx*", 'X'));
...
TEST(String, EscapeUnEsacpe) {
  string str = "0ccc\n\r\a\b\t\v\"\'xxx\x05\x06 d\001";
  string s1 = CEscapeString(str);
  ASSERT_EQ("0ccc\\n\\r\\a\\b\\t\\v\\\"\\\'xxx\\x05\\x06 d\\x01", s1);
  string s2 = CUnescapeString(s1);
  ASSERT_EQ(str, s2);

  str = "\?";
  s1 = CEscapeString(str);
  ASSERT_EQ("\\\?", s1);
  s2 = CUnescapeString(s1);
  ASSERT_EQ(str, s2);
}
