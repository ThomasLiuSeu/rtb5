...
class CountingStats {
 ...
 public:
  ...
  T GetTotal() const {
    typename MutexType::Locker locker(m_mutex);
    return m_total;
  }

  void Increase(int64_t current_time = -1) {
    int64_t timestamp = (current_time != -1) ? current_time : GetTimeStampInMs() /
                        1000;
  ...
  void GetStatsResult(CountingStatsResult<T>* result,
...
class MeanStats {
 ...
 public:
  ...
  void GetStatsResult(MeanStatsResult<T>* result,
                      int64_t current_time = -1) const {
    int64_t timestamp = (current_time != -1) ? current_time : GetTimeStampInMs() /
                        1000;
  ...
  void GetHistoricalItem(std::vector<StatsItem<T> >* items,
                         int64_t current_time = -1) const {
    int64_t timestamp = (current_time != -1) ? current_time : GetTimeStampInMs() /
                        1000;
    items->clear();
...
#endif // COMMON_BASE_TIMED_STATS_H
