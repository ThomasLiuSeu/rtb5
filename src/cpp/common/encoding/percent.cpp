...
void PercentEncoding::EncodeAppend(const StringPiece& input,
                                   std::string* output) {
...
void PercentEncoding::EncodeUriAppend(const StringPiece& input,
                                      std::string* output) {
  static const ByteSet unchanged("ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                 "abcdefghijklmnopqrstuvwxyz"
                                 "0123456789!#$&'()*+,-./:;=?@_~");
...
void PercentEncoding::EncodeUriTo(const StringPiece& input,
                                  std::string* output) {
  output->clear();
...
void PercentEncoding::EncodeUriComponentAppend(const StringPiece& input,
    std::string* output) {
  static const ByteSet unchanged("ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                 "abcdefghijklmnopqrstuvwxyz"
                                 "0123456789!'()*-._~");
...
void PercentEncoding::EncodeUriComponentTo(const StringPiece& input,
    std::string* output) {
  output->clear();
...
bool PercentEncoding::DecodeAppend(const StringPiece& input,
                                   std::string* output) {
  for (size_t i = 0; i < input.size(); ++i) {
    uint8_t ch = 0;
    if (input[i] == '%') {
