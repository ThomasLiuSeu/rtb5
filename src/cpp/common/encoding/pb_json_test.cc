...
TEST_F(PbJsonTest, ProtoMessageToJson) {
  ...
  test_message.set_query("×ãÇò±¦±´some");
  ...
  reflection->SetString(&test_message, field_query_ext, "º¬Òåquery");
...
TEST_F(PbJsonTest, JsonToProtoMessage) {
  ...
  EXPECT_EQ("×ãÇò±¦±´some", test_message.query());
